...
export default function TodoStatus({todo,showDate,tasks,onCompleted,onUncompleted,updateTracker}): React.JSX.Element {
...
// console.log( taskList );
    ...
    // console.error( "==============>>", val, min, max, todo.rangeMin, todo.rangeMax )
    ...
    return <View>
        <View key="1" style={todoViewStyle.week}>
        ...
        <View key="2" style={ trackerGraphStyle.container } >
            { taskList.map( (t,idx) => {
                const valid = !!(t && t.value !== null && t.value !== undefined && !isNaN(t.value))
                let value = valid ? t.value : 0
                value = (value - todo.rangeMin) / ( todo.rangeMax - todo.rangeMin );
                // console.log( ":::", t.value )
                return <View key={idx} style={{ ...trackerGraphStyle.bar, width: w+"%", }} >
                        { valid && 
                            <>
